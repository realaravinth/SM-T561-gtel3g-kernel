...
static int sprdfb_mmap(struct fb_info *info,struct vm_area_struct *vma)
{
	...
	if(NULL == info){
		printk(KERN_ERR "[LCD] sprdfb: sprdfb_ioctl error. (Invalid Parameter)");
		return -1;
	}

	dev = info->par;
	printk("[LCD] sprdfb: sprdfb_mmap,vma=0x%x\n",vma);
...
static int setup_fb_mem(struct sprdfb_device *dev, struct platform_device *pdev)
{
	...
	if(use_reserve_mem){
		ret = of_property_read_u32_array(pdev->dev.of_node, "sprd,fb_mem",
				reserve_mem, 2);
		if(0 != ret){
			printk(KERN_ERR "[LCD] sprdfb: Failed to got framebuffer memory from dt file\n");
		}
	}
#else
	...
	if(!use_reserve_mem){
		...
		if (!addr) {
			printk(KERN_ERR "[LCD] sprdfb: Failed to allocate framebuffer memory\n");
			return -ENOMEM;
		}
		printk(KERN_INFO "[LCD] sprdfb: got %d bytes mem at 0x%x\n", len, addr);
		...
		printk("[LCD] sprdfb: setup_fb_mem--smem_start:%lx,len:%d,reserved len:%d\n",dev->fb->fix.smem_start,len,reserve_mem[1]);
		addr =  (uint32_t)ioremap(dev->fb->fix.smem_start, len);
		if (!addr) {
			printk(KERN_ERR "[LCD] sprdfb: Unable to map framebuffer base: 0x%08x\n", addr);
			return -ENOMEM;
		}
		dev->fb->fix.smem_len = len;
		dev->fb->screen_base = (char*)addr;
...
static int sprdfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *fb)
{
	...
	if(0 == dev->enable){
		printk(KERN_ERR "[LCD] sprdfb: [%s]: Invalid Device status %d", __FUNCTION__, dev->enable);
		return -1;
	}
		...
		unsigned long arg)
{
	...
	case SPRD_FB_DISPLAY_OVERLAY:
		pr_debug(KERN_INFO "[LCD] sprdfb: [%s]: SPRD_FB_DISPLAY_OVERLAY\n", __FUNCTION__);
		...
		if (copy_from_user(&local_overlay_display, argp, sizeof(local_overlay_display))){
			printk("[LCD] sprdfb: DISPLAY_OVERLAY copy failed!\n");
			return -EFAULT;
		}
	...
	case FBIO_WAITFORVSYNC:
		pr_debug(KERN_INFO "[LCD] sprdfb: [%s]: FBIO_WAITFORVSYNC\n", __FUNCTION__);
	...
	case SPRD_FB_CHANGE_FPS:
		{
			...
			if (result) {
				pr_err("[LCD] %s: copy_from_user failed", __func__);
				return result;
			}
			pr_info("[LCD] %s: fps will be changed to %d via ioctl\n",
					__func__, fps);
			result = sprdfb_chg_clk_intf(dev,
					SPRDFB_DYNAMIC_FPS, fps);
			if (result) {
				pr_err("[LCD] %s: fps is set fail. fps=%d, ret=%d\n",
						__func__, fps, result);
				return result;
			}
			break;
	...
	case SPRD_FB_IS_REFRESH_DONE:
		pr_debug(KERN_INFO "[LCD] sprdfb: [%s]: SPRD_FB_IS_REFRESH_DONE\n", __FUNCTION__);
	...
	default:
		printk(KERN_INFO "[LCD] sprdfb: [%s]: unknown cmd(%d)\n", __FUNCTION__, cmd);
		break;
	}

	pr_debug(KERN_INFO "[LCD] sprdfb: [%s]: return %d\n",__FUNCTION__, result);
	return result;
}
...
static int sprdfb_power(struct sprdfb_device *dev, int on)
{
	...
	if (on) {
		dev->ctrl->resume(dev);
#ifdef CONFIG_LCD_ESD_RECOVERY
#ifndef CONFIG_LCD_ESD_RECOVERY_BY_TSP
...
static int sprdfb_probe(struct platform_device *pdev)
{
	...
	if(SPRDFB_MAINLCD_ID == dev->dev_id){
		...
		printk("[LCD] sprdfb: set g_dispc_base_addr = 0x%x\n", g_dispc_base_addr);
#endif
#ifdef CONFIG_FB_SC8825
	}else{
	...
	if (ret) {
		printk(KERN_ERR "[LCD] sprdfb: sprdfb_probe register framebuffer fail.\n");
		goto cleanup;
	}
